...
4. Vincular el gestor de **Agent Skills** (`skills`) para Google Antigravity IDE en `~/.local/bin/skills`.
5. Configurar el servicio systemd de reanudación automática tras suspensión.
6. Recargar Niri al vuelo si ya está ejecutándose.

---

## 🤖 Gestor de Agent Skills para Antigravity IDE

Tus dotfiles integran un gestor profesional, idempotente y con análisis de seguridad Zero-Trust para desplegar 42 Agent Skills comunitarias curadas:

```bash
skills
```

El comando abre un panel interactivo con las siguientes opciones:
- `skills status`: Muestra las skills instaladas y su estado.
- `skills install`: Descarga, audita e instala las 42 skills en `~/.gemini/config/skills/`.
- `skills update`: Actualiza repositorios comunitarios y aplica novedades.
- `skills scan`: Ejecuta auditoría estática contra riesgos de seguridad (`curl|bash`, `rm -rf /`, exfiltración).
- `skills list`: Muestra el catálogo detallado de skills disponibles por categorías.
- `skills uninstall <skill>`: Desinstala una skill conservando backups y skills locales.
...
├── vscode/                     # Preferencias visuales de Antigravity IDE y VSCodium
├── skills/                     # Motor de instalación y auditoría de Agent Skills
├── scripts/                    # Utilidades ejecutables (~/.local/bin: skills, sysupdate.sh, etc.)
...
| **`Mod + C`** | 📝 **Editor de Código** (Antigravity IDE / VSCodium) |
...
- **Gestor de Skills de IA:** `skills` (Antigravity IDE Agent Skills Ecosystem)
